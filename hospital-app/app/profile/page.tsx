...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-slate-50 to-teal-50 p-6" style={{ animation: 'fadeIn 0.6s ease-out' }}>
      <style>{`
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6" style={{ animation: 'slideInDown 0.6s ease-out' }}>
          <div className="flex items-center gap-4">
            <div>
              <h2 className="text-3xl md:text-4xl font-extrabold bg-gradient-to-r from-teal-600 to-emerald-600 bg-clip-text text-transparent">Hospital Profile</h2>
              <p className="text-sm text-slate-600 mt-1">Manage hospital-level data, capacity and contacts.</p>
            </div>
          </div>

          <div className="flex gap-3 items-center">
            <Link href="/resources" className="px-4 py-2 bg-white border border-teal-200 text-teal-600 rounded-lg shadow-sm hover:shadow-md hover:border-teal-300 transition-all duration-300 font-medium">Resources</Link>
            <button onClick={() => setEditing((s) => !s)} className="px-4 py-2 bg-gradient-to-r from-teal-600 to-teal-700 text-white rounded-lg hover:shadow-lg transition-all duration-300 shadow font-medium">{editing ? 'Cancel' : 'Edit'}</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4 mb-2" style={{ animation: 'slideInUp 0.6s ease-out 0.1s both' }}>
            <div className="bg-white rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col border border-slate-100 hover:border-teal-200">
              <div className="text-xs text-slate-500 font-semibold uppercase tracking-wide">Available Beds</div>
              <div className="flex items-center justify-between mt-3">
                <div className="text-3xl font-bold text-teal-600">{animBeds}</div>
              ...
              <div className="mt-4 w-full bg-slate-100 rounded-full h-3 overflow-hidden">
                <div className="h-3 bg-gradient-to-r from-teal-400 to-teal-600 transition-all duration-700" style={{ width: `${Math.round((profile.beds_available/profile.beds_total)*100)}%` }} />
              </div>
              <div className="text-xs text-slate-400 mt-2">{Math.round((profile.beds_available/profile.beds_total)*100)}% utilized</div>
            </div>

            <div className="bg-white rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col border border-slate-100 hover:border-emerald-200">
              <div className="text-xs text-slate-500 font-semibold uppercase tracking-wide">ICU Available</div>
              <div className="flex items-center justify-between mt-3">
                <div className="text-3xl font-bold text-emerald-600">{animICU}</div>
                <div className="text-xs text-slate-500">of {profile.icu_total}</div>
              </div>
              <div className="mt-4 w-full bg-slate-100 rounded-full h-3 overflow-hidden">
                <div className="h-3 bg-gradient-to-r from-emerald-400 to-emerald-600 transition-all duration-700" style={{ width: `${Math.round((profile.icu_available/profile.icu_total)*100)}%` }} />
              </div>
          ...
          <div className="lg:col-span-2 bg-white p-8 rounded-2xl shadow-md border border-slate-100" style={{ animation: 'slideInUp 0.6s ease-out 0.2s both' }}>
            ...
            {editing && (
              <div className="mt-6 flex gap-3">
                <button onClick={save} className="px-6 py-2 bg-gradient-to-r from-teal-600 to-teal-700 text-white rounded-lg shadow hover:shadow-lg transition-all duration-300 font-medium">Save Changes</button>
          ...
          <aside className="bg-white p-8 rounded-2xl shadow-md space-y-4 border border-slate-100" style={{ animation: 'slideInUp 0.6s ease-out 0.3s both' }}>
            ...
            <div>
              ...
              <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
                <div className="h-3 bg-teal-500 transition-all" style={{ width: `${Math.round((profile.beds_available/profile.beds_total)*100)}%` }} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
                <div className="h-3 bg-emerald-500 transition-all" style={{ width: `${Math.round((profile.icu_available/profile.icu_total)*100)}%` }} />
              </div>
            </div>
